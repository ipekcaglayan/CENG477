...
void parser::Scene::loadFromXml(const std::string &filepath)
{
    ...
    Mesh mesh;
    float numberOfVertices = vertex_data.size();
    std::vector<Vec3f> normalsBeforeAvg(numberOfVertices);
    std::vector<int> numberOfNormals(numberOfVertices);
    while (element)
    {
        ...
        Face face;
        Vec3f face_normal;
        vertex_normals = new float[vertex_data.size() * 3];
        vertex_pos = new float[vertex_data.size() * 3];
        while (!(stream >> face.v0_id).eof())
        {
            stream >> face.v1_id >> face.v2_id;
            Vec3f v0 = vertex_data[--face.v0_id];
            Vec3f v1 = vertex_data[--face.v1_id];
            Vec3f v2 = vertex_data[--face.v2_id];
            face_normal = ((v0 - v1).CrossProduct(v0 - v2)).Normalized();
            normalsBeforeAvg[face.v0_id] += face_normal;
            normalsBeforeAvg[face.v1_id] += face_normal;
            normalsBeforeAvg[face.v2_id] += face_normal;
            numberOfNormals[face.v0_id]++;
            numberOfNormals[face.v1_id]++;
            numberOfNormals[face.v2_id]++;

            mesh.faces.push_back(face);
        }
        stream.clear();
        face_number += mesh.faces.size();
    ...
    {
        if (numberOfNormals[i])
        {
            avgNormal = normalsBeforeAvg[i] / numberOfNormals[i];
        }
        else{
            avgNormal={0,0,0};
        ...
        vertex_normals[3 * i] = avgNormal.x;
        vertex_normals[3 * i + 1] = avgNormal.y;
        vertex_normals[3 * i + 2] = avgNormal.z;
        vertex_pos[3 * i] = vertex_data[i].x;
        vertex_pos[3 * i + 1] = vertex_data[i].y;
        vertex_pos[3 * i + 2] = vertex_data[i].z;
    }
